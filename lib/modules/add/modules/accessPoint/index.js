...
class accessPoint extends BaseModule{
    ...
    createNewAccessPoint(){
        ...
        const createFile = (filePath, fileContent, config) => {
        ...
        const customiceTemplate = () => {
            ...
            // className
            let classNameValue = self.parameters[2].value.toUpperCase();
        ...
        const createDir = () => {
            ...
            super.printSpacedMessage('creating new module in '+ self.destinationFolderName + '/' + dirPath + ' ...')
            fs.mkdir(fullPath, {recursive:true}, function(error){
                if (error) {
                    ...
                    createFile( "." + self.destinationFolderName + '/' + dirPath + '/' + dirPath + ".js", 
                                content);
                }
            });
        };
    ...
    init(...args){
        ...
        if(args[1]==undefined || args[2]==undefined){
            return super.printSpacedMessage("please make sure you entered the correct parameters. example: --verbs=[GET, POST, PUT] --route=exampleroute")
        }
        else{
